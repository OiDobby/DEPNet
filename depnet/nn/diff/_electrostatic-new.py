...
from depnet.datasets import CHARGES_KEY, ELECTROSTATIC_ENERGY_KEY, TOTAL_CHARGE_KEY
...
class Qeq(GraphModuleMixin, torch.nn.Module):
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        ### edit and add lines ###
        species_idx = species_idx.to(device)	#new
        self.sigma = self.sigma.to(device)	#new
        sigmas = self.sigma[species_idx]	#new
        #########################
        gammas = torch.sqrt(
        ...
        ### edit and add line ###
        coeffs_transposed = torch.transpose(coeffs.clone(), 0, 1)
        coeffs += coeffs_transposed
        #########################
