...
from depnet.data import AtomicData
...
class AtomicInMemoryDataset(AtomicDataset):
    ...
    def __init__(
        ...
        if self.data is None:
            self.data, self.fixed_fields, include_frames = torch.load(self.processed_paths[0], weights_only=False,)
    ...
    def processed_file_names(self):
        ...
        # ===== train/valid split
        suffix = ""
        if self.extra_fixed_fields.get("cache_by_filename", False):
            base = splitext(basename(self.file_name))[0]
            suffix = f"_{base}"
        elif "cache_suffix" in self.extra_fixed_fields:
            suffix = f"_{self.extra_fixed_fields['cache_suffix']}"

        return [f"{r_max}_{dtype}{suffix}_data.pt"]
        # =====
        # ===== original code lines
        '''
    ...
    def statistics(
        ...
        node_selector = torch.as_tensor(np.isin(self.data.batch.numpy(), selector.numpy()))
        ...
        # in1d change to isin for numpy > 2.0
...
class ASEDataset(AtomicInMemoryDataset):
    ...
    def get_atoms(self):
        ...
        atoms = aseread(self.raw_dir + "/" + self.raw_file_names[0], **self.ase_args)
        
        if isinstance(atoms, list):
            for atom in atoms:
                atom.info["electrode_potential"] = atom.info.get("electrode_potential", None)

            return atoms

        atoms.info["electrode_potential"] = atoms.info.get("electrode_potential", None)
    ...
    def get_data(self):
        ...
        frames = self.include_frames if self.include_frames is not None else range(len(atoms_list))
