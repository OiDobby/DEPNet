...
from depnet.datasets import TOTAL_CHARGE_KEY
from depnet.data import AtomicDataDict
from depnet.data._util import _TORCH_INTEGER_DTYPES
...
class AtomicData(Data):
    ...
    def from_ase(cls, atoms, r_max, **kwargs):
        ...
        # ===== additional block
        '''
        if AtomicDataDict.CHARGES_KEY in add_fields:
            # Shape transformation (N,) -> (N, 1)
            if add_fields[AtomicDataDict.CHARGES_KEY].ndim == 1:
                add_fields[AtomicDataDict.CHARGES_KEY] = add_fields[AtomicDataDict.CHARGES_KEY][:, None]
        else:
            # If CHARGES_KEY is not in add_fields, retrieve it from atoms.arrays
            for k in ("charge", "charges"):  # Consider both "charge" and "charges" keys
                if k in atoms.arrays:
                    charges = atoms.arrays[k]
                    if charges.ndim == 1:  # Convert shape from (N,) -> (N, 1)
                        charges = charges[:, None]
                    add_fields[AtomicDataDict.CHARGES_KEY] = charges
                    break
        '''
        # Retrieve charges from add_fields if available; otherwise, check atoms.arrays
        charges = add_fields.get(AtomicDataDict.CHARGES_KEY)
        if charges is None:  # If charges are not in add_fields, check atoms.arrays
            charges = next((atoms.arrays[k] for k in ("charge", "charges") if k in atoms.arrays), None)

        # Ensure charges have the correct shape (N,) -> (N,1)
        if isinstance(charges, np.ndarray) and charges.ndim == 1:
            charges = charges[:, None]

        # Store charges in add_fields if found
        if charges is not None:
            add_fields[AtomicDataDict.CHARGES_KEY] = charges

        add_fields[AtomicDataDict.ELECTRODE_KEY] = atoms.info.get("electrode_potential", None)
        # ===== 
    ...
    def to_ase(self) -> Union[List[ase.Atoms], ase.Atoms]:
        ...
        ### editted code line ###
        do_calc = energy is not None or force is not None or AtomicDataDict.STRESS_KEY is not None
        #########################
