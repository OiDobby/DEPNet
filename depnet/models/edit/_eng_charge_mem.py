...
from depnet.datasets import CHARGES_KEY, ELECTROSTATIC_ENERGY_KEY, TOTAL_CHARGE_KEY, STRESS_KEY
...
def EnergyChargeModel(**shared_params) -> SequentialGraphNetwork:
    ...
    num_short_layers = shared_params.pop("num_short_layers", 0)
    num_Qeq_layers = shared_params.pop("num_Qeq_layers", 0)
            ...
            if num_Qeq_layers > 0:
                layers["total_energy_with_qeq"] = (
                    ...
                    dict(
                        ...
                        use_sc=use_sc,
                        linear_sc=linear_sc,
                        num_Qeq_layers=num_Qeq_layers,
                    ),
                )
            else:
                layers["conv_to_output_hiden"] = AtomwiseLinear
                layers["total_energy_with_qeq"] = (
                    EwaldQeq,
                    dict(
                        scale=energy_scale,
    ...
    if num_short_layers > 0:
        for layer_i in range(num_short_layers):
            layers[f"layer{layer_i}_convnet_2"] = (
                ConvNetLayer,
                dict(use_sc=use_sc, linear_sc=linear_sc, debug=True),
            )
    else:
...
def QeqForceModel(**shared_params) -> GraphModuleMixin:
    energy_charge_model = EnergyChargeModel(**shared_params)
    return QeqForceOutput(model=energy_charge_model)
