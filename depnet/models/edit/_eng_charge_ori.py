...
def EnergyChargeModel(**shared_params) -> SequentialGraphNetwork:
    ...
    for layer_i in range(num_layers):
        ...
        layers[f"layer{layer_i}_convnet"] = (
            ConvNetLayer,
            dict(use_sc=use_sc, linear_sc=linear_sc),
        )
        ########################
        if use_nonlocal:
    ...
    non_linear_irreps_out = shared_params.get("conv_to_output_hidden_irreps_out")
...
def StressChargeModel(**shared_params) -> GraphModuleMixin:
    force_charge_model = ForceChargeModel(**shared_params)
    return StressForceOutput(model=force_charge_model)
