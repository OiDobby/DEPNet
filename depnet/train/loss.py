...
from depnet.datasets import ELECTRODE_KEY, AREA_KEY
...
class Loss:
    ...
    def __call__(self, pred: dict, ref: dict):
        ...
        for key in self.coeffs:
            # ===== Custom loss for U_pred
            if key == ELECTRODE_KEY:
                '''
                # ===== Debug: 직접 RMSE 계산 =====
                rmse_manual = torch.sqrt(torch.mean((U_pred - U_true) ** 2)).item()
                print("[DEBUG: LOSS] U_pred[:3]:", U_pred[:3])
                print("[DEBUG: LOSS] U_true[:3]:", U_true[:3])
                print("[DEBUG: LOSS] RMSE (manual):", rmse_manual)
                # ==================================
                '''
                U_pred = pred["U_sum"].squeeze(-1)
                ref_val = ref[key].squeeze(-1).clone() if ref[key].ndim == 2 else ref[key].clone()
                area_val = ref[AREA_KEY].squeeze(-1).clone() if ref[AREA_KEY].ndim == 2 else ref[AREA_KEY].clone()
                U_true = ref_val * area_val
...
class LossStat:
    def __init__(self, keys, loss_instance=None):
