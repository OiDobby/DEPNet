...
#import nequip
from depnet.nequip_nn import GraphModuleMixin
import depnet

CONFIG_KEY: Final[str] = "config"
NEQUIP_VERSION_KEY: Final[str] = "depnet_version"
...
def main(args=None):
    ...
    ### additional code block (to use gpu) ###
    build_parser.add_argument(
        "--device",
        help="Device to load the model on (e.g., 'cpu' or 'cuda'). Default is 'cpu'.",
        type=str,
        default="cpu",
    )
    ##########################################
    ...
    device = torch.device(args.device)  ### additional line (Use the specified device)
    # TODO: configurable?
    logging.basicConfig(level=logging.INFO)

    if args.command == "info":
        #model, metadata = load_deployed_model(args.model_path)  ### original line
        model, metadata = load_deployed_model(args.model_path, device=device)  ### edited line
        del model
        ...
        try:
            #model = torch.jit.load(model_path, map_location=torch.device("cpu"))  ### original line
            model = torch.jit.load(model_path, map_location=device)  ### edited line
            model_is_jit = True
            logging.info("Loaded TorchScript model")
        except RuntimeError:
            # ^ jit.load throws this when it can't find TorchScript files
            #model = torch.load(model_path, map_location=torch.device("cpu"))  ### original line
            model = torch.load(model_path, map_location=device)  ### edited line
            if not isinstance(model, GraphModuleMixin):
                raise TypeError(
                    "Model contained object that wasn't a NequIP model (depnet.nequip_nn.GraphModuleMixin)"
                )
            logging.info("Loaded pickled model")
        #model = model.to(device=torch.device("cpu"))  ### original line
        model = model.to(device=device)  ### edited line

        logging.info("try script(model)")
        ...
        logging.info("compiled model success")
        ...
        # Deploy
        metadata: dict = {NEQUIP_VERSION_KEY: depnet.__version__}
