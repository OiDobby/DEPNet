...
from depnet.scripts.train import regression_by_species
from depnet.utils import Config, dataset_from_config, load_file
...
def restart(file_name, config, mode="update"):
    ...
    logging.info(f"Loaded dictionary: {dictionary}")
    ...
    if config.wandb:
        ...
        if config.wandb_resume:
            from depnet.nequip_utils.wandb import resume
