...
from depnet.data import AtomicData, AtomicInMemoryDataset
from depnet.datasets._keys import CHARGES_KEY, SYMBOLS_KEY, TOTAL_CHARGE_KEY
...
class FGHDNNPDataset(AtomicInMemoryDataset):
    ...
    def _get_atomicdata(self, atoms_dict) -> AtomicData:
        ...
        add_fields[CHARGES_KEY] = np.expand_dims(
            np.array(atoms_dict.get(CHARGES_KEY, np.zeros(len(atoms_dict.get(SYMBOLS_KEY, []))))), axis=1
        ) ### edited line
        ...
        ### additional code STRESS_KEY ###
        stress = atoms_dict.get(AtomicDataDict.STRESS_KEY)
        if stress is None:
            stress = np.zeros((3, 3))
        add_fields[AtomicDataDict.STRESS_KEY] = np.array(stress)
        #add_fields[AtomicDataDict.STRESS_KEY] = np.array(atoms_dict.get(AtomicDataDict.STRESS_KEY, np.zeros((3, 3))))
        ##################################
