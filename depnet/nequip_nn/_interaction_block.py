...
class InteractionBlock(GraphModuleMixin, torch.nn.Module):
    ...
    def __init__(
        ...
        use_sc=False,       ### additional tag
        linear_sc=False,    ### additional tag
        ...
        self.use_sc = use_sc ### additional line
        self.linear_sc = linear_sc ### additional line
        print(f"Initialized with use_sc: {self.use_sc}, linear_sc: {self.linear_sc}")
        ...
        if self.use_sc:
            if self.linear_sc:
                self.LIN_sc = Linear(
                    irreps_in=feature_irreps_in,  # Input irreps (same as node features)
                    irreps_out=feature_irreps_out,  # Output irreps (same as node features)
                    internal_weights=True,
                    shared_weights=True,
                )
                self.TP_sc = None
            else:
                self.TP_sc = FullyConnectedTensorProduct(
                    feature_irreps_in,
                    self.irreps_in[AtomicDataDict.NODE_ATTRS_KEY],
                    feature_irreps_out,
                )
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        #print("Node feature:", data[AtomicDataDict.NODE_FEATURES_KEY].shape)
        #print("Node attrs:", data[AtomicDataDict.NODE_ATTRS_KEY].shape)
        ...
        if self.use_sc:
            if self.linear_sc:
                if self.LIN_sc is None:
                    raise RuntimeError("Linear_sc is not initialized")
                sc = self.LIN_sc(x)
            elif hasattr(self, 'TP_sc'):
                if self.TP_sc is None:
                    raise RuntimeError("TP_sc is not initialized")
                sc = self.TP_sc(x, data[AtomicDataDict.NODE_ATTRS_KEY])
        #######################
